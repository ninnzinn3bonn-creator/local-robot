...
画面左にカメラ映像、右に接続状態、操作卓、AI認識、3Dデジタルツインが表示されます。右上の `接続` でローカル操作卓の状態を再取得できます。チャット履歴とテキスト入力は `チャット履歴・入力` の折りたたみ領域にしまっています。通常はWake Wordで開始できます。`話しかける` ボタンを押すと、次の発話だけWake Wordなしで会話を開始できます。応答後は、折りたたみ領域内のメーターにWake Wordなしで続けて話せる残り秒数が表示されます。`会話終了` を押すと、その連続会話モードを手動で終えます。
