...
ブラウザで開くURLは `http://127.0.0.1:8765` です。状態APIは `http://127.0.0.1:8765/api/state`、カメラJPEGは `http://127.0.0.1:8765/api/frame.jpg`、VOICEVOXは `http://127.0.0.1:50021`、Ollamaは `http://127.0.0.1:11434` を使います。

停止は、起動したPowerShellで `Ctrl+C` が基本です。Web UIだけを確実に止める場合:
...
詳しくは [docs/RUN_LOCAL.md](docs/RUN_LOCAL.md) の「起動・停止・URL早見表」を見てください。
